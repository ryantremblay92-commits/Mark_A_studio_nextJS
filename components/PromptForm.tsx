...
// Fix: Updated to use AspectRatio values from types.ts and included all members
const aspectRatioDisplayNames: Record<AspectRatio, string> = {
  [AspectRatio.BANNER]: 'Landscape (16:9)',
  [AspectRatio.STORY]: 'Portrait (9:16)',
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  // Fix: Use correct AspectRatio value (BANNER instead of LANDSCAPE)
  const [aspectRatio, setAspectRatio] = useState<AspectRatio>(
    initialValues?.aspectRatio ?? AspectRatio.BANNER,
  );
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      // Fix: Use correct AspectRatio value (BANNER instead of LANDSCAPE)
      setAspectRatio(initialValues.aspectRatio ?? AspectRatio.BANNER);
  ...
  useEffect(() => {
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      ...
      // Fix: Use correct AspectRatio value (BANNER instead of LANDSCAPE)
      setAspectRatio(AspectRatio.BANNER);
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          ...
          <textarea
            ...
            className="flex-grow bg-transparent focus:outline-none resize-none text-base text-gray-200"
            rows={1}
          ...
          <button
            ...
            className={`p-2 rounded-lg hover:bg-gray-700 transition-colors ${
              isSettingsOpen ? 'text-indigo-400 bg-gray-700' : 'text-gray-400'
            }`}
          ...
          <button
            ...
            className="p-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-lg transition-colors flex-shrink-0"
            title={tooltipText}
